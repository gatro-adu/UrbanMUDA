...
class AgentInput(BaseModel):
    ...
    configs: Optional[list[dict[str, Any]]|list[str]] = None
...
class MonitoredWebSocket:
    def __init__(self, websocket, task_id):
        self.websocket = websocket
        self.task_id = task_id
        self.task_name: str  # 创建时先不设置task_name，等收到init消息后设置，避免因为task_name长度太长，导致websocket连接失败
    ...
    async def close(self):
        if not self.is_closed:
            print(f"Closing connection for task {self.task_name}") if len(self.task_name) < 200 else None
            self.is_closed = True
            await self.websocket.close()

@app.websocket("/ws")  # 前端每次new一个WebSocket连接，就会创建一个websocket连接，这里就是client端
async def websocket_endpoint(websocket: WebSocket, task_id: str):  # 会创建main_task_id以及sub_task_id
    monitored_ws = MonitoredWebSocket(websocket, task_id)
    await monitored_ws.websocket.accept()
    data = await monitored_ws.websocket.receive_text()
    message = json.loads(data)
    if message.get('type') == 'init':
        task_name = message.get('task_name')
        monitored_ws.task_name = task_name
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("fastapi_server.app_test_agent:app", host="0.0.0.0", port=8001, reload=True)
