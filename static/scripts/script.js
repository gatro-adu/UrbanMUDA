...
async function sendChatCommand() {
  ...
  // 设置默认测试指令
  if (!command) {
    command = "在集乳站东侧10米呈正三角队形部署蓝军3辆PLZ-45自行火炮";
  }
  ...
  connectToParentTask(data.task_id, data.task_name, taskBubble, systemBubble);  // 初始连接
}
...
function connectToParentTask(taskId, taskName, userBubbleDiv, systemBubbleDiv) {
  ...
  // 创建WebSocket连接
  const ws = new WebSocket(`ws://${location.host}/ws?task_id=${taskId}`);
  wsConnections[taskId] = ws;
  ws.onopen = () => {
    // 在连接成功后，发送 taskName 给后端
    ...
    try {
      ...
      if (data_json.type === "init") {
        ...
        if (data_json.task_id !== taskId) {
          createTaskWindow(data_json.task_id, data_json.task_name);  // 如果发来init消息，则新建websocket连接
        }
...
// tasksContainer.addEventListener('scroll', function() {  // 此处没有output!!!
//   // 检查用户是否滚动到底部
