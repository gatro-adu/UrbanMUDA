import sys  # 导入问题都是sys.path不对的问题
sys.path.append('..')
import math
import uuid
...
from scripts.llm_load import llm_deploy_rule
...
@mcp.tool()
def get_nearby_geoentities(
    loc_name: Annotated[str, Field(description="Name of the location")], 
    if_no_dingxing=False, if_no_dingliang=False
) -> list:
    ...
    class GeoEntity(BaseModel):
        ...
        tag: str
        geometry: str
        # bearing: Annotated[Optional[str], Field(exclude=True)]=None
    ...
    def is_linestring_like(geom):
        return geom.geom_type in ["LineString", "LinearRing"]
    nearby = []
    triplets = []
    geo_pair = []
    for location_feature in location_feature:
        ...
        if location_tag in {'footway', 'steps', 'primary', 'secondary', 'tertiary', 'residential'} and is_linestring_like(location_geo):
            location_temp = list(location_geo.coords)
            # print('location_geo: ', location_temp)
            location_bearing = calculate_bearing(*location_temp[0], *location_temp[-1])
            nearby.append(Info(target_geoentity=GeoEntity(name=location_name, tag=location_tag, geometry=str(location_geo), bearing=str(location_bearing))))
        else:
        ...
        for _, nearby_feature in nearby_features.iterrows():
            ...
            geo_pair.append({location_geo.wkt, nearby_geo.wkt})
            # (修改点)
            if if_no_dingxing:
                relationship = Relation(rcc="unknown", distance=searcher.get_distance(location_geo, nearby_geo), azimuth=searcher.calculate_azimuth(location_geo, nearby_geo))
            elif if_no_dingliang:
                relationship = Relation(rcc=searcher.get_rcc(location_geo, nearby_geo), distance="unknown", azimuth="unknown")
            else:
                relationship = Relation(rcc=searcher.get_rcc(location_geo, nearby_geo), distance=searcher.get_distance(location_geo, nearby_geo), azimuth=searcher.calculate_azimuth(location_geo, nearby_geo))
            # (/修改点)
            # (修改点)
            if nearby_tag in {'footway', 'steps', 'primary', 'secondary', 'tertiary', 'residential', 'service'} and is_linestring_like(nearby_geo):
                nearby_temp = list(nearby_geo.coords)
                # print('location_geo: ', location_temp)
                nearby_bearing = calculate_bearing(*nearby_temp[0], *nearby_temp[-1])
                nearby_equip = GeoEntity(name=nearby_name, tag=nearby_tag, geometry=str(nearby_geo), bearing=str(nearby_bearing))
            else:
            ...
            # (/修改点)
            # (修改点)
            triplets.append(f"({nearby[-1].target_geoentity.model_dump(exclude_none=True)}, {nearby[-1].nearby[-1].relationship.model_dump(exclude_none=True)}, {nearby[-1].nearby[-1].nearby_geoentity.model_dump(exclude_none=True)})")
            # (/修改点)
    if len(str(triplets))>word_limit: raise NodeInterrupt(f'{loc_name}附近的地理实体太多。请重新输入指令')
    result = f"=== 找到{loc_name}附近地理实体 ===\n\n"+"\n".join(triplets)
    return result
def get_nearby_geoentities_no_compress(
    loc_name: Annotated[str, Field(description="Name of the location")], 
    if_no_dingxing=False, if_no_dingliang=False
) -> list:
    '''Tool to obtain nearby geoentities using location name'''
    ...
    class Info(BaseModel):
        ...
        nearby: Optional[list[Nearby]]=[]
    # (/修改点)
    location_feature = searcher.similar_match_geoentities(loc_name)  # feature: name, tag, geometry
    if len(location_feature)==0: 
        raise NodeInterrupt(f'未找到{loc_name}。请重新输入')
    def is_linestring_like(geom):
        return geom.geom_type in ["LineString", "LinearRing"]
    ...
    for location_feature in location_feature:
        ...
        location_tag = location_feature[1]
        location_geo = location_feature[2]
        if location_tag in {'footway', 'steps', 'primary', 'secondary', 'tertiary', 'residential'} and is_linestring_like(location_geo):  # 如果是线形则可以计算本身的偏角
            location_temp = list(location_geo.coords)
            # print('location_geo: ', location_temp)
            location_bearing = calculate_bearing(*location_temp[0], *location_temp[-1])
            nearby.append(Info(target_geoentity=GeoEntity(name=location_name, tag=location_tag, geometry=str(location_geo), bearing=str(location_bearing))))
        ...
        for _, nearby_feature in nearby_features.iterrows():
            ...
            if nearby_tag in {'footway', 'steps', 'primary', 'secondary', 'tertiary', 'residential', 'service'} and is_linestring_like(nearby_geo):
                ...
                nearby_equip = GeoEntity(name=nearby_name, tag=nearby_tag, geometry=str(nearby_geo))
            # nearby_equip = GeoEntity(name=nearby_name, tag=nearby_tag, geometry=str(nearby_geo))
            nearby[-1].nearby.append(Nearby(nearby_geoentity=nearby_equip))
            # (/修改点)
            # (修改点)
            target = nearby[-1].target_geoentity.model_dump(exclude_none=True)
            neighbor = nearby[-1].nearby[-1].nearby_geoentity.model_dump(exclude_none=True)
            triplets.append(f"({target} -> {neighbor})")
            # triplets.append(f"({nearby[-1].target_geoentity.model_dump(exclude_none=True)}, {nearby[-1].nearby[-1].relationship.model_dump(exclude_none=True)}, {nearby[-1].nearby[-1].nearby_geoentity.model_dump(exclude_none=True)})")
            # (/修改点)
    if len(str(triplets))>word_limit: raise NodeInterrupt(f'{loc_name}附近的地理实体太多。请重新输入指令')
    result = f"=== 找到{loc_name}附近地理实体 ===\n\n"+"\n".join(triplets)
    return result
def get_nearby_geoentities_no_dingliang(
    loc_name: Annotated[str, Field(description="Name of the location")], 
    if_no_dingxing=False, if_no_dingliang=False
) -> list:
    '''Tool to obtain nearby geoentities using location name'''
    if_no_dingxing = False
    if_no_dingliang = True
    # (修改点)
    class GeoEntity(BaseModel):
        name: Optional[str]
        tag: str
    ...
    result = f"=== 找到{loc_name}附近地理实体 ===\n\n"+"\n".join(triplets)
    return result
def get_nearby_geoentities_no_dingxing(
    loc_name: Annotated[str, Field(description="Name of the location")]
) -> list:
    '''Tool to obtain nearby geoentities using location name'''
    if_no_dingxing = True
    if_no_dingliang = False
    # (修改点)
    class GeoEntity(BaseModel):
        name: Optional[str]
        tag: str
...
async def get_deploy_rules(name: Annotated[str, Field(description="Name of the simentity")]) -> str:
    ...
    rules = await llm_deploy_rule.ainvoke(f"请基于下列兵力实体帮我生成3条部署规则: {name}")
...
#     n: Annotated[int, Field(description="Number of the formation")],
#     d: Annotated[float, Field(description="Distance of every two vertexes, in meters")],
#     desc: Annotated[str, Field(description="Other description of the formation")],
#     coord: Annotated[list[float], Field(description="Coordinates of central point of formation")],
#     ) -> str:
async def get_formation(
    coord: list[float]=Field(..., description="Coordinates of central/leader point of formation"),
    desc: str=Field(..., description="具体队形描述，包括位置坐标、排列方式、装备配置等必要且完备的元素")
    ) -> str:
    ...
    # }
    graph = react_agent(
        tools=[searcher.formation_polygon, searcher.formation_line, searcher.formation_wedge],
        prompt=prompt,
        memory=False
    )
    # user_input = f'{name}，中心点/领队在{coord}，部署数量{n}，两两间距{d}米，其他要求为{desc}'
    user_input = desc + f'中心点/领队在{coord}'
