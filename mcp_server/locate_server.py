...
equip_infos = [
    ...
    {
        'ID': 'M1A2_BP_C__11A97C0346A77C166E467BA4FA88D1F9', 
        'Name': '99A主战坦克__3', # 西南侧
        'TableID': 11960, 
        'TeamID': 0, 
    ...
    {
        'ID': 'M1A2_BP_C__11A97C0346A77C166E467BA4FA88D1F10', 
        'Name': '59式中型坦克', # 东南侧
        'TableID': 11960, 
        'TeamID': 0, 
...
@mcp.tool()
def get_nearby_simentities(loc_name: Annotated[str, Field(description="Name of the location")]) -> list:
    '''Tool to obtain the nearby simentities using the location name'''
    ...
    for location_feature in location_feature:
        ...
        nearby_features = searcher.get_nearby_simentities(location_geo, scale, equip_infos)
        ...
        for _, nearby_feature in nearby_features.iterrows():
            ...
            nearby_equip = SimEntity(id=nearby_id, name=nearby_name, team=nearby_team, pos=str(get_coordinates(nearby_geo).tolist()[0]))
            nearby[-1].nearby.append(Nearby(relationship=relationship, nearby_simentity=nearby_equip))
            # triplets.append(f"({nearby[-1].target_area}, {nearby[-1].nearby[-1].relationship}, {nearby[-1].nearby[-1].nearby_simentity})")
            triplets.append(f"(<{nearby[-1].target_geoentity.name}, {nearby[-1].target_geoentity.tag}>, {nearby[-1].nearby[-1].relationship}, <{nearby[-1].nearby[-1].nearby_simentity.name}, {nearby[-1].nearby[-1].nearby_simentity.team}>)")
    if len(str(triplets))>word_limit: raise NodeInterrupt(f'{loc_name}附近的兵力实体太多。请重新输入指令')
    result = f"\n=== 找到{loc_name}附近兵力实体 ===\n"+"\n".join(triplets)
    return result

@mcp.tool()
def get_simentity(name: Annotated[str, Field(description="Name of the simentity")]) -> str:
    '''Tool to get the info of target simentity using name'''
